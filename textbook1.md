...
・[新しいプロジェクト] で ```Visual C# > Android > Android アプリ(Xamarin)``` を選択します。  
・[名前] に「```XAMediaRecorder```」を入力します。[場所] は好みの場所があれば変更してください。  
・[OK] をクリックします。  
・[New Android App] ダイアログで [からのアプリ] を選択します。  
・[OK] をクリックします。  
...
・タイトル「フォルダーの結合」のダイアログで「すべての項目に適用」チェックをONにし、[はい] をクリックします。  
・タイトル「対象ファイルが存在します」のダイアログで「すべての項目に適用」チェックをONにし、[はい] をクリックします。  
