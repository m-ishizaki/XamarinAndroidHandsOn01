...
namespace XAMediaRecorder
{
    ...
    public class MediaCodecWrapper
    {
                 ...
                 int flags)
        {
            ...
            if (size > 0 && mAvailableInputBuffers.Any())
            {
                ...
                {
                    throw new MediaCodecWrapper.WriteException(Java.Lang.String.Format(Locale.Us,
        ...
        public bool peekSample(MediaCodec.BufferInfo out_bufferInfo)
        {
            ...
            update();
            bool result = false;
            if (mAvailableOutputBuffers.Any())
            {
        ...
        public void popSample(bool render)
        {
            ...
            if (mAvailableOutputBuffers.Any())
            {
                int index = mAvailableOutputBuffers.Dequeue();
        ...
        private void update()
        {
            ...
            while ((index = mDecoder.DequeueInputBuffer(0)) != (int)MediaCodec.InfoTryAgainLater)
            {
                mAvailableInputBuffers.Enqueue(index);
            }
            ...
            while ((index = mDecoder.DequeueOutputBuffer(info, 0)) != (int)MediaCodec.InfoTryAgainLater)
            {
                switch (index)
                {
                    ...
                    default:
                        ...
                        if (index >= 0)
                        {
                            ...
                            mAvailableOutputBuffers.Enqueue(index);
                        }
                        else
                        {
